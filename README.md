...
class PretrainedImageEncoder(nn.Module):
    """Encodes image input using a pretrained timm model."""
    def __init__(self, model_name: str = "resnet50"):
        super(PretrainedImageEncoder, self).__init__()
        self.model = timm.create_model(model_name, pretrained=True, num_classes=0)  # No classification head
...
class MultimodalFusion(nn.Module):
    ...
    def __init__(self, hidden_dim: int):
        ...
        self.fc = nn.Linear(4352, hidden_dim)  # 768 (text) + 768 (speech) + 2048 (image) + 768 (video)
    ...
    def forward(self, encodings: List[torch.Tensor]) -> torch.Tensor:
        ...
        """
        concatenated = torch.cat(encodings, dim=1)  # Concatenate along the feature dimension
...
class HumanoidMultimodalModel(nn.Module):
    ...
    def forward(
        ...
        text_encoding = self.text_encoder(text_input)  # Shape: [batch_size, seq_length, hidden_dim]
        speech_encoding = self.speech_encoder(speech_input)  # Shape: [batch_size, seq_length, hidden_dim]
        image_encoding = self.image_encoder(image_input)  # Shape: [batch_size, hidden_dim]
        video_encoding = self.video_encoder(video_input)  # Shape: [batch_size, hidden_dim]
